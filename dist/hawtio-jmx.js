...
(function (Jmx) {
    ...
    Jmx.AttributesController = Jmx._module.controller("Jmx.AttributesController", ["$scope", "$element", "$location",
        "workspace", "jolokia", "jmxWidgets", "jmxWidgetTypes", "$templateCache", "localStorage", "$browser", '$timeout',
        function ($scope, $element, $location, workspace, jolokia, jmxWidgets, jmxWidgetTypes, $templateCache, localStorage, $browser, $timeout) {
            $scope.searchText = '';
            $scope.nid = 'empty';
            $scope.selectedItems = [];
            $scope.lastKey = null;
            ...
            $scope.onUpdateAttribute = function () {
                ...
                if (mbean) {
                    jolokia.setAttribute(mbean, key, value, Core.onSuccess(function (response) {
                        $timeout(function () { return $scope.notification = { type: 'success', message: 'Updated successfully' }; }, 0);
                        $timeout(function () { return $scope.notification = null; }, 8000);
                    }));
                }
            };
...
var JVM;
(function (JVM) {
    JVM._module.controller("JVM.DiscoveryController", ["$scope", "$timeout", "localStorage", "jolokia",
        function ($scope, $timeout, localStorage, jolokia) {
            ...
            function doConnect(agent) {
                if (!agent.url) {
                    $timeout(function () { return $scope.notification = { type: 'warning', message: 'No URL available to connect to agent' }; }, 0);
                    $timeout(function () { return $scope.notification = null; }, 8000);
                    return;
                }
